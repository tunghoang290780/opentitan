...
class pwm_base_vseq extends cip_base_vseq #(
  ...
  constraint pwm_regs_c {
    ...
    foreach (pwm_regs.blink_param_x[i]) {
      pwm_regs.blink_param_x[i] inside {[0 : 2**(pwm_regs.dc_resn + 1)]};
    }
    foreach (pwm_regs.blink_param_y[i]) {
      pwm_regs.blink_param_y[i] inside {[0 : 2**(pwm_regs.dc_resn + 1)]};
    }
    foreach (pwm_regs.duty_cycle_a[i]) {
      pwm_regs.duty_cycle_a[i] dist {
        [0:2**(pwm_regs.dc_resn + 1) - 1] :/1,
         2**(pwm_regs.dc_resn + 1) :/1,
        [2**(pwm_regs.dc_resn + 1) + 1 : 2**(pwm_regs.dc_resn + cfg.seq_cfg.pwm_max_dc_resn)] :/2
      };
    }
  ...
  // functions
  virtual function void update_pwm_config();
    // derived params
